...
#if 0
static ft_ty
exttype(const jstr_literal_ty *fname)
{
	const char *ext = jstr_memrchr(fname->data, '.', fname->size);
	if (ext != NULL && *++ext != '\0') {
		...
		for (i = 0; i < sizeof(textv) / sizeof(*textv); ++i)
			if (!jstr_strcmpeq_loop(ext, textv[i]))
				return FT_TEXT;
		for (i = 0; i < sizeof(binv) / sizeof(*binv); ++i)
			if (!jstr_strcmpeq_loop(ext, binv[i]))
				return FT_BINARY;
	}
	return FT_UNKNOWN;
}
...
JSTR_IO_FTW_FUNC(callback_file, ftw, args)
{
	...
	const jstr_literal_ty fn = { ftw->dirpath, (unsigned int)ftw->dirpath_len };
	if (jstr_chk(process_file(a->t, a->buf, &fn, ftw->st, &a->find, &a->rplc)))
		JSTR_RETURN_ERR(JSTR_RET_ERR);
	return JSTR_RET_SUCC;
}
