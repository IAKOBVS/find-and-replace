             ...
             const jstr_literal_ty *rplc)
{
	...
	if (G.confirm_pass && (G.mode & MODE_CONFIRM)) {
		size_t matches = 0;
		jstr_ret_ty ret = confirm_scan_file(t, buf, fname, find, rplc, &matches);
		...
		if (matches > 0 || (isatty(STDIN_FILENO) && isatty(STDOUT_FILENO)))
			file_pushback(&G.files, fname, st, buf);
		return ret;
	}
	jstr_ret_ty ret = process_buffer(t, buf, fname, st, find, rplc);
	return ret;
}
