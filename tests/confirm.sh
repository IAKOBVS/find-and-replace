...
t_confirm_interactive_enter_invalid() {
	td=$1; printf 'hello world\n' > "$td/f"
	pdrive --noready --phase 0d --phase 04@150 -- '[' HI -c -i -R "$td/f"
	if [ "$(cat "$td/rc")" = 1 ] && [ "$(cat "$td/f")" = 'hello world' ]; then
		echo PASS > "$td/result"
	else
		echo "FAIL: rc=[$(cat "$td/rc")] file=[$(cat "$td/f")]" > "$td/result"
...
t_confirm_interactive_alt_j_scroll() {
	td=$1; printf 'aaa\nbbb\nccc\nddd\neee\n' > "$td/f"
	# Alt-J scrolls the selection down; assert TUI doesn't crash and Enter works
pdrive --phase 1b6a --phase 1b6a --phase 0d --tail 'y\n' -- a X -g -c -i "$td/f"
	if [ "$(cat "$td/f")" = 'XXX
bbb
ccc
ddd
...
t_confirm_interactive_alt_k_scroll() {
	td=$1; printf 'aaa\nbbb\nccc\n' > "$td/f"
	# Alt-J down twice, Alt-K back up once, Enter, y
pdrive --phase 1b6a --phase 1b6a --phase 1b6b --phase 0d --tail 'y\n' -- a X -g -c -i "$td/f"
	if [ "$(cat "$td/f")" = 'XXX
bbb
ccc' ]; then
		echo PASS > "$td/result"
...
# Regression: Alt-J in the confirm TUI stopped scrolling when G.total_lines
...
# terminal.  Alt-J 15 times must still work (total_lines=40), and Enter
# must apply the replacement to all lines.
t_confirm_interactive_scroll_past_visend() {
	td=$1
	i=1; while [ "$i" -le 20 ]; do printf 'aaa\n'; i=$((i+1)); done > "$td/f"
	phases=""
	i=1; while [ "$i" -le 15 ]; do phases="$phases --phase 1b6a@50"; i=$((i+1)); done
